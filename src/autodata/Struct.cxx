...
std::string AutoStructNoTH::ToJSON()
{
    std::string json("{ \"typename\" : \"");
    json += m_typename + "\", \"data\" : "
            + Poco::Dynamic::Var::toString( m_struct ) + " }";
    return json;
}

void AutoStructNoTH::FromJSON(const std::string& json)
{
    ...
    Poco::Dynamic::Var tmp = set->getObject( "data" );
    m_struct = Poco::Dynamic::Var::parse( Poco::Dynamic::Var::toString( tmp ) );
}

void AutoStructNoTH::Load( Poco::Data::Session& session )
{
...
void AutoStructNoTH::Save( Poco::Data::Session& session )
{
    //Lazy ID creation
    if( m_id.empty() )
    {
...
Poco::Dynamic::Struct<std::string>::ConstIterator AutoStructNoTH::begin() const
{
    return m_struct.extract< Poco::Dynamic::Struct< std::string > >().begin();
}

Poco::Dynamic::Struct<std::string>::ConstIterator AutoStructNoTH::end() const
{
    return m_struct.extract< Poco::Dynamic::Struct< std::string > >().end();
}

void AutoStructNoTH::CreateTable( Poco::Data::Session& session )
{
    Poco::Data::Statement stmt( session );
    stmt << "CREATE TABLE IF NOT EXISTS " << m_typename << "(";
    //'id' TEXT, 'Col1' TEXT, 'Col2' REAL, 'Col3' INTEGER, 'Col4' BOOLEAN)"

    // Poco::Dynamic::Var can only distinguish String, Numeric, and Integer
...
bool AutoStructNoTH::TableExists( Poco::Data::Session& session )
{
    bool exists = false;
